...
- 在主界面显示像素风对话框，进入主界面时随机显示一句，点击后会切换到剩余句子
- 对话框会自动跟随 `yc_ui` 的位置、大小与左右朝向
- 当 `yc_ui` 靠左时，对话框使用原图；当 `yc_ui` 靠右时，`dialog_box.png` 会自动左右翻转
- 对话框宽度会随当前句子长度自动调整，人物缩放时，对话框与文字也会同步缩放
...
- 主界面对话框不再单独编辑位置与大小，而是自动跟随 `yc_ui`
- 对话框底图是否翻转由 `yc_ui` 当前位于屏幕左侧还是右侧自动决定
- 对话框文字不会镜像
