...
- 主界面的提示文字会在人物上方偏外侧横向渐显，约 1 秒完全显示
- 提示文字会根据 `yc_ui` 的尺寸同步缩放，并尽量保持约 1:3 的比例关系
- 长句会自动换成两行；遇到 `，` 时会优先断行，并尽量避开人物本体，不遮挡 `yc_ui`
...
- 主界面的提示文字不单独编辑位置与大小，而是自动跟随 `yc_ui`
- 主界面的下一句切换由点击 `yc_ui` 触发
- 提示文字不会镜像，但会根据人物位置自动选择更合适的显示侧
